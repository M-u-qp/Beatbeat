...
private const val TRACKS_POPULARITY_ORDER = "popularity_month"
...
class GenreSearchViewModel @Inject constructor(
    ...
    fun onTrackClicked(itemTrackUI: ItemTrackUI) {
        playerManager.playTrack(
            trackId = itemTrackUI.id,
            audioUrl = itemTrackUI.audio,
            title = itemTrackUI.name,
            imageUrl = itemTrackUI.image
        )
    }

    fun loadSearchResultToTrack(tags: String) {
        GenericPager.paginate(
            pagingSourceFactory = {
                getSearchResultToTrackUseCase.pagingInvoke(
                    order = TRACKS_POPULARITY_ORDER,
                    tags = tags
                )
            },
